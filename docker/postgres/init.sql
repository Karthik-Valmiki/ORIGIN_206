...
CREATE TABLE inspections (
    ...
    rule_id                 UUID NOT NULL REFERENCES lmpc_rules(id)         ON DELETE RESTRICT,
...
CREATE TABLE findings (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    inspection_id   UUID NOT NULL REFERENCES inspections(id) ON DELETE CASCADE,
    rule_id         UUID          REFERENCES lmpc_rules(id)  ON DELETE SET NULL,
...
-- Removed rule applicability index
...
-- ==========================================
-- SCHEMA ADDITIONS (FROM PART 1 PLAN)
-- ==========================================
...
ALTER TABLE extracted_fields
    ADD COLUMN confidence FLOAT CHECK (confidence BETWEEN 0.0 AND 1.0);
